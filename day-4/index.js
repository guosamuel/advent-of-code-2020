...
fs.readFile('./input.txt', 'utf8', (err, data) => {
  ...
  for (let i = 0; i < parsedData.length; i++) {
    ...
    for (let j = 0; j < parsedData[i].length; j++) {
      const splittedBySpace = parsedData[i][j].split(" ")
      placeholder.push(...splittedBySpace)
    }
...
fs.readFile('./input.txt', 'utf8', (err, data) => {
  ...
  const validateFunctions = {
    'byr': year => validateBYR(year),
    'iyr': year => validateIYR(year),
    'eyr': year => validateEYR(year),
    'hgt': height => validateHGT(height),
    'hcl': color => validateHCL(color),
    'ecl': color => validateECL(color),
    'pid': id => validatePID(id)
  }

  function validateBYR(year) {
  ...
  function validateHGT(height) {
    ...
    const intHeight = Number(height.substring(0, height.length-2))
    if (units === 'cm') return intHeight >= 150 && intHeight <= 193
    if (units === 'in') return intHeight >= 59 && intHeight <= 76
    return false
  }
